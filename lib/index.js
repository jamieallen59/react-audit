...
import { promptForRulesToAdd, parseCliArgs, writeRulesToFile } from './cli'
...
export async function runReactAudit(args) {
  ...
  const rulesToAdd = await promptForRulesToAdd(cliOptions, rulesNotFound)

  // 4. Add rules to lint file
  await writeRulesToFile(rulesToAdd)
}
